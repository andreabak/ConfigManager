...
class CheckNoneNonOptionalFieldsMixin:
    ...
    This allows us not to worry about optional dataclass fields order and
    related dataclasses restrictions, so that optional fields checking is
    delayed to instantiation time using type annotations checking,
    ...
    def __post_init__(self):
        ...
        for field in dataclass_fields(self):
            if getattr(self, field.name, None) is None and not is_optional_type(
                ...
                raise TypeError(
                    f"{field.name} was not specified for {self.__class__.__name__}"
                )
...
class ConfigSectionBase(CheckNoneNonOptionalFieldsMixin):
    ...
    Any declared subclass should be decorated with `@dataclass` to ensure they're
    correctly defined as dataclasses.
    ...
    It can load values from a `configparser.SectionProxy` and do some basic
    typecasting from str values and vice-versa convert an existing instance
    to a `configparser.SectionProxy`, converting all fields to str.
    """

    section_name: ClassVar[str] = None
    """Defines the section name as serialized to a `configparser.SectionProxy` name.
    This can be set to the special placeholder `AUTO_NAME` and, when used together 
    with a `ConfigBase` instance, section_name will be automatically set from the
    associated `ConfigBase`'s field name (this is set at class level, and there is
    no immediate collision checking mechanism)"""
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if getattr(cls, "section_name", None) is None:
            raise SyntaxError(
                "Subclasses of `ConfigSection` must specify a class variable "
                '"section_name" as a string representing the config section name '
                'or `AUTO_NAME` to generate the "section_name" from a '
                "`ConfigBase` class' field name"
            )

    def __post_init__(self):
        """
        Dataclass initialization method. We make sure that at initialization time
        the "section_name" class attribute is a valid name and not `AUTO_NAME` anymore.
        """
        if self.section_name is AUTO_NAME:
            raise ValueError(
                f"{self.__class__.__name__} has uninitialized section_name"
            )
        super().__post_init__()

    @classmethod
    def convert_field_types_from_strings(
        cls, source_data: Mapping[str, str]
    ) -> MutableMapping:
        """
        Convert a mapping of str: str to a mapping of class fields name: values,
        doing some basic typecasting
        :param source_data: The source data
        :return: The converted mapping representing fields data
        :raise KeyError: If any of the keys of source_data are not known fields names
        """
        ...
        if unknown_source_nameset:
            raise KeyError(
                f"Unknown config field names in source data: "
                f'{", ".join(unknown_source_nameset)}'
            )
    ...
    def from_config_section(
        ...
        :raise ValueError: If the name of the "section" object does not match
        the class' "section_name" attribute
        """
        if cls.section_name != section.name:
            raise ValueError(
                f'ConfigSection {cls.__name__} name "{cls.section_name}"'
    ...
    def to_config_section(
        self, parser: configparser.RawConfigParser
    ) -> configparser.SectionProxy:
        """
        Convert a `ConfigSectionBase` instance to a config section object,
        doing some basic typecasting to str.
        :param parser: A `configparser.RawConfigParser` instance to initialize
            the `SectionProxy`
        ...
        for (
            name,
            value,
        ) in data.items():  # TODO: Maybe skip values that == default for that field
...
def config_dataclass(cls: Type[ConfigBase]) -> Type[ConfigBase]:
    ...
    It converts the class to a dataclass, then checks that all fields are of
    `ConfigSectionBase` type, then ensures that each section name matches the field name,
    ...
    if wrong_type_fields:
        raise TypeError(
            f"Some fields of ConfigBase subclass {cls.__name__}"
    ...
    if mismatched_field_names:
        raise ValueError(
            f"Some field names of ConfigBase subclass {cls.__name__}"
...
class ConfigBase(CheckNoneNonOptionalFieldsMixin):
    ...
    Any declared subclass should be decorated with `@config_dataclass` to ensure
    they're correctly defined.
    The rationale is to have a dataclass only with fields of `ConfigSectionBase`
    type representing the configuration sections. The field name must match the
    section name, or `AUTO_NAME` must be used for "section_name" of the
    `ConfigSectionBase` class.
    It provides functionality to load and save a configuration from an .ini file
    using `configparser` and apply the loaded values to the sections
    ensuring all the names match in the defined dataclasses.
    """

    _config_parser_factory: ClassVar[
        Callable[..., configparser.RawConfigParser]
    ] = configparser.ConfigParser
    """The callable "factory" that returns an instance of
    `configparser.RawConfigParser` when called,
    that is then used to parse the .ini file(s)"""

    config_parser: InitVar[configparser.RawConfigParser] = None
    """An init-only argument that passes the `RawConfigParser` instance
    used to initialize the dataclass"""

    config_path: InitVar[Optional[PathType]] = None
    """An init-only argument that specifies the path of the .ini file
    ...
    def __post_init__(
        ...
        :param config_parser: The `configparser.RawConfigParser` used to
            initialize this instance
        :param config_path: An optional path of the .ini file used by
            `configparser` to load the values from
        """
        super().__post_init__()
    ...
    @classmethod
    def load(
        cls, path: Optional[Union[PathType, Sequence[PathType]]] = None, **parser_kwargs
    ) -> ConfigBase:
        """
        Create a `ConfigBase` instance, optionally loading values from a .ini file
        :param path: An optional path or sequence of paths of .ini files
            to load the config values from
        :param parser_kwargs: Optional keyword arguments to be passed to
        ...
        for field in cls.get_section_fields():
            if not issubclass(field.type, ConfigSectionBase):
                raise TypeError(
                    f"Members of {cls.__name__} must be subclasses of "
                    f"{ConfigSectionBase.__name__}, got: {repr(field)}"
                )
            if field.name in parser:
                section = field.type.from_config_section(parser[field.name])
            else:
                # Will raise an exception if Section class has non-optional fields
                section = field.type()
    ...
    def save(
        self, path: Optional[PathType] = None, encoding="utf8", **write_kwargs
    ) -> None:
        """
        Save an .ini file using the configparser used to initialize the instance
        :param path: The .ini file path to save the config to. If omitted,
            it will try to be derived from the .ini file path used at init time.
        ...
        :raise ValueError: If no path was provided nor any path could be extracted
            from initialization arguments
        """
        if path is None:
            path = self._config_path
        if path is None:
            raise ValueError(
                f'Argument "path" was not specified, and {self.__class__.__name__} '
        ...
        for field in self.get_section_fields():
            ...
            if not isinstance(section, ConfigSectionBase):
                raise TypeError(
                    f"Members of {repr(self)} must be subclasses of "
                    f"{ConfigSectionBase.__name__}, got: {repr(field)}"
                )
            self._config_parser[field.name] = section.to_config_section(
                self._config_parser
            )
